...
$sql = "SELECT ps_product_lang.name, ps_manufacturer.name as manufacturer, ps_product.id_product, ps_product_lang.description_short
...
$sth->execute
    or die "SQL Error: $DBI::errstr\n";
     ...
     while (@row = $sth->fetchrow_array) {
     	$row[0]=~ s/\[.*\]//;
          print "UPDATE ps_product_lang SET ps_product_lang.meta_description = \"$row[3]\" WHERE ps_product_lang.id_product = $row[2];\n";
	   }
